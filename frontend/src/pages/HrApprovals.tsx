...
export default function HrApprovals() {
  ...
  const handleReview = async () => {
    ...
    setReviewing(true);
    try {
      await hrReviewLeave(token!, confirmModal.id, {
  ...
  // HR sees all final statuses + pending_hr
  const allStatuses: LeaveStatus[] = ["pending_hr", "approved", "rejected"];
  ...
  const filterTabs: FilterStatus[] = ["all", "pending_hr", "approved", "rejected"];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">HR Approvals</h2>
          <p className="text-sm text-slate-500 mt-0.5">
            Review and finalise leave requests forwarded from managers
          </p>
        </div>
        <button
          onClick={fetchLeaves}
